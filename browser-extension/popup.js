// VeroLente AI Popup Script v2.0
...
async function runPopupVerification(data) {
  ...
  try {
    ...
    console.error('[VeroLente AI Popup]', err);
...
function showPopupResult(result, originalData) {
  ...
  document.getElementById('popup-share-btn').addEventListener('click', () => {
    const text = `"${claim.substring(0, 100)}" - Truth Score: ${score}% | Verified by VeroLente AI\n${BASE_URL}/result/${result.id || ''}`;
...
function showHelp() {
  ...
  resultArea.innerHTML = `
    <div style="padding:14px;background:#f0fdf4;border-radius:10px;border:1px solid #bbf7d0;">
      <p style="font-size:12.5px;font-weight:700;color:#166534;margin:0 0 8px">How to Use VeroLente AI</p>
      <ul style="font-size:12px;color:#15803d;margin:0;padding-left:16px;line-height:1.8">
        ...
        <li><strong>Right-click</strong> any text, image, or link → Verify with VeroLente AI</li>
